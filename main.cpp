...
const char* kDefaultOutputFile = "E:/Projects/EmojiRender/font/out.png";
...
class DrawContext {
public:
    DrawContext() : pos_(0), width_(0), height_(0) {}
...
struct FaceOptions {
    ...
    FaceOptions() : pixel_size(kDefaultPixelSize), load_flags(0), render_mode(FT_RENDER_MODE_NORMAL) {}
};

class FreeTypeFace {
public:
    FreeTypeFace(const std::string& font_file) : font_file_(font_file), options_(), face_(nullptr) {
    ...
    bool CalculateBox(uint32_t codepoint, uint32_t& width, uint32_t& height) {
        ...
        height = std::max(height, static_cast<uint32_t>(face_->glyph->metrics.height >> 6));
        return true;
    }
...
private:
    ...
    void SetupNormalFont() { error_ = FT_Set_Pixel_Sizes(face_, 0, options_.pixel_size); }
    void SetupColorFont() {
        ...
        for (int i = 1; i < face_->num_fixed_sizes; ++i) {
            int ndiff = std::abs(options_.pixel_size - face_->available_sizes[i].width);
            if (ndiff < diff) {
                best_match = i;
                diff = ndiff;
            }
...
class PngWriter {
public:
    PngWriter(const std::string& outfile) : outfile_(outfile), png_(nullptr), info_(nullptr) {
    ...
    bool Write(uint8_t* rgba, int width, int height) {
        ...
        png_set_IHDR(png_, info_, width, height, kDepth, PNG_COLOR_TYPE_RGB_ALPHA, PNG_INTERLACE_NONE,
                     PNG_COMPRESSION_TYPE_DEFAULT, PNG_FILTER_TYPE_DEFAULT);
        png_init_io(png_, fp_);
        png_byte** row_pointers = static_cast<png_byte**>(png_malloc(png_, height * sizeof(png_byte*)));
        uint8_t* src = rgba;
        for (int y = 0; y < height; ++y) {
            png_byte* row = static_cast<png_byte*>(png_malloc(png_, width * kBytesPerPixel));
            row_pointers[y] = row;
...
public:
    ...
    bool SetText(const std::u32string& text) { return UTF8ToCodepoint(text); }
    bool Execute() {
        CalculateImageSize();
        Draw();
        return Output();
...
private:
    bool UTF8ToCodepoint(const std::u32string& text) {
        for (const auto& it : text) {
            codepoints_.push_back(it);
        }
        return true;
    }
    ...
    bool Output() {
        ...
        return writer.Write(draw_context_.Bitmap(), draw_context_.Width(), draw_context_.Height());
    }
...
void Finish() { FT_Done_FreeType(gFtLibrary); }

void Usage() {
    std::cout << "Usage: clfontpng font1.ttf [font2.ttf ...] text" << std::endl;
    std::exit(1);
}

bool ParseArgs(App& app, int argc, char** argv) {
    // app.AddFont("E:/Projects/EmojiRender/font/NotoEmoji-Regular.ttf");
    app.AddFont("E:/Projects/EmojiRender/font/NotoColorEmoji-Regular.ttf");
    return app.SetText(U"😂❤💋👏");
}
